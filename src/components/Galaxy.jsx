...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    try {
      const renderer = new Renderer({
        ...
        preserveDrawingBuffer: false // FIXED: Set to false to prevent white flash
      });
      ...
      // FIXED: Better transparent background handling
      if (transparent) {
        gl.enable(gl.BLEND);
        gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA); // FIXED: Better blend function for transparency
      ...
      function update(t) {
        ...
        // FIXED: Check if we're still mounted
        if (!ctnDom.current) {
        ...
        try {
          // FIXED: Clear with transparent background properly
          if (transparent) {
      ...
      if (mouseInteraction) {
        ...
        ctn.addEventListener('mousemove', mouseMoveListener);
        ctn.addEventListener('mouseleave', mouseLeaveListener);
      }

    } catch (error) {
      console.error('Galaxy WebGL initialization error:', error);
    }
    ...
    return () => {
      ...
      // FIXED: Proper cleanup with stored references
      if (resizeListener) {
      ...
      if (currentRenderer) {
        ...
        if (mouseInteraction && currentCtn) {
          if (mouseMoveListener) {
            currentCtn.removeEventListener('mousemove', mouseMoveListener);
          }
          if (mouseLeaveListener) {
            currentCtn.removeEventListener('mouseleave', mouseLeaveListener);
          }
        }
        
        // Remove canvas
        if (gl.canvas && gl.canvas.parentNode === currentCtn) {
          currentCtn.removeChild(gl.canvas);
        }
  ...
  return <div ref={ctnDom} className="w-full h-full relative" style={{ background: 'transparent' }} {...rest} />;
}